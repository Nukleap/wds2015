...
public class DateUtils {
	...
	public static String DATE_FORMAT_MAIN = "dd-mm-yyyy HH:mm:ss";
	public static String DATE_APT_DATE = "MMM dd yyyy hh:mma";
	...
	public static Date parseDate(String content){
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				DATE_FORMAT_MAIN, Locale.getDefault());
		try{
			Date d = dateFormat.parse(content);
			return d;
		}catch(Exception e){
	...
	public static String parseAptDate(String content){
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				DATE_APT_DATE, Locale.getDefault());
		try{
			Date d = dateFormat.parse(content);
	...
	/*public static long getHours(Calendar from,Calendar to){
		return TimeUnit.MILLISECONDS.toHours(to.getTimeInMillis()-from.getTimeInMillis());
	}*/
	...
	/*public static Calendar addHours(Date oldDate,int duration){
	    Date newDate = new Date(oldDate.getTime() + TimeUnit.MINUTES.toMillis(duration));
	    Calendar cal = Calendar.getInstance();
	    cal.setTime(newDate);
	    return cal;
	}*/
	...
	public static boolean isToday(Calendar cal){
		...
		if((c.get(Calendar.MONTH)==month)&&(c.get(Calendar.YEAR)==year)&&(c.get(Calendar.DATE)==day)){
			return true;
		}else{
			return false;
		}
	...
	public static String format(Calendar dt,boolean addTime){
		...
		if(addTime){
			addtime = dt.get(Calendar.HOUR_OF_DAY)+":"+dt.get(Calendar.MINUTE);
		}
		return dt.get(Calendar.DAY_OF_MONTH)+" "+getMonthAbr(dt.get(Calendar.MONTH))+" "+addyear+" "+addtime;
	}
